...
- **AND** aggregate and per-package statement and branch coverage remain strictly above 95 percent
...
aggregate and per-package statement and branch coverage remain strictly above
95 percent with current bound evidence, the native source gate passes,
...
before a trusted release. The module aggregate and every canonical package
SHALL maintain statement and branch coverage strictly above 95 percent under
the single repository coverage policy. Every production package SHALL appear
...
- **WHEN** one native platform reports a wholly unexecuted production package or
  an aggregate or package ratio at or below the floor
