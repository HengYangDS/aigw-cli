...
func TestRunRejectsIncompleteAndExistingOutput(t *testing.T) {
	...
	err := run([]string{"replay", "--source", ".", "--revision", "HEAD", "--output", output, "--actor-name", "Actor", "--actor-email", "actor@example.com", "--signing-key", "key", "--allowed-signers", allowed})
	if err == nil || !strings.Contains(err.Error(), "output already exists") {
		t.Fatalf("existing output error = %v", err)
	}
}

func TestReplayFlagParserRejectsTrailingArguments(t *testing.T) {
	option := signedReplayFixture(t)
	args := []string{"--source", option.source, "--revision", option.revision, "--output", option.output, "--actor-name", option.actorName, "--actor-email", option.actorEmail, "--signing-key", option.signingKey, "--allowed-signers", option.allowedSigners, "trailing"}
	if err := runReplay(args); err == nil || !strings.Contains(err.Error(), "required") {
		t.Fatalf("trailing argument=%v", err)
	}
}

func TestReplayFlagParserRejectsMalformedFlag(t *testing.T) {
	if err := runReplay([]string{"--unknown"}); err == nil {
		t.Fatal("unknown replay flag accepted")
	}
}
...
func TestExecuteReportsInvalidInvocation(t *testing.T) {
	...
	option := signedReplayFixture(t)
	args := []string{"replay", "--source", option.source, "--revision", option.revision, "--output", option.output, "--ref", option.ref, "--actor-name", option.actorName, "--actor-email", option.actorEmail, "--signing-key", option.signingKey, "--signing-program", option.signingProgram, "--allowed-signers", option.allowedSigners}
...
func main() {
	...
	case "rev-parse-symbolic:rev-parse":
		for _, argument := range args {
			if argument == "--symbolic-full-name" {
				os.Exit(1)
			}
		}
	case "rev-parse-commit:rev-parse":
		for _, argument := range args {
			if filepath.Base(argument) == "main^{commit}" {
				os.Exit(1)
			}
		}
