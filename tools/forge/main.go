// Command forge owns Forge-neutral history, provenance, tag, synchronization,
// and branch-closeout contracts. Provider credentials and trust inputs remain
// explicit invocation inputs; this command never discovers personal identity.
package main
...
func run(args []string) error {
	...
	case "commits":
		return runCommitProvenance(args[1:])
	case "tag":
		return runTagSignature(args[1:])
	case "tags":
		return runTagNamespace(args[1:])
	case "sync":
	...
	default:
		return fmt.Errorf("unknown forge command: %s", args[0])
	}
}

func runReplay(args []string) error {
	var option options
	flags := flag.NewFlagSet("forge replay", flag.ContinueOnError)
