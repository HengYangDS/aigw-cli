// Command civerify owns portable CI and local source acceptance orchestration.
package main
...
var sourceCommands = []command{
	{Name: "go", Args: []string{"run", "./tools/cicontract", "toolchain", "."}},
	{Name: "go", Args: []string{"run", "./tools/releasekit", "validate-toolchain", "go.mod"}},
	{Name: "go", Args: []string{"run", "./tools/releasekit", "validate-release-sources"}},
	{Name: "go", Args: []string{"run", "./tools/architecture", "--root", "."}},
	{Name: "go", Args: []string{"test", "./tools/architecture"}},
	{Name: "go", Args: []string{"run", "./tools/coveragegate", "--race"}},
	{Name: "go", Args: []string{"vet", "./..."}},
	{Name: "go", Args: []string{"tool", "staticcheck", "-checks=all,-ST1000,-ST1005", "./..."}},
	{Name: "go", Args: []string{"tool", "errcheck", "./..."}},
	{Name: "go", Args: []string{"run", "./tools/repositorycheck", "--root", ".", "go-format"}},
	{Name: "go", Args: []string{"run", "./tools/repositorycheck", "--root", ".", "product-surface"}},
	{Name: "go", Args: []string{"run", "./tools/repositorycheck", "--root", ".", "english-text"}},
	{Name: "go", Args: []string{"run", "./tools/repositorycheck", "--root", ".", "credentials"}},
	{Name: "go", Args: []string{"test", "./tools/repositorycheck"}},
	{Name: "go", Args: []string{"run", "./tools/repositorycheck", "--root", ".", "governance"}},
	{Name: "go", Args: []string{"test", "./internal/selfupdate", "./tools/releasekit"}},
	{Name: "go", Args: []string{"run", "./tools/cicontract", "pipeline", "."}},
	{Name: "go", Args: []string{"run", "./tools/cicontract", "github-verify", "."}},
	{Name: "go", Args: []string{"run", "./tools/cicontract", "github-release", "."}},
...
func run(args []string, stdout io.Writer, runner commandRunner) error {
	if len(args) == 0 {
		return errors.New("usage: civerify <source|native>")
	}
	switch args[0] {
	case "source":
		if len(args) != 1 {
			return errors.New("usage: civerify source")
		}
	...
	case "native":
		flags := flag.NewFlagSet("civerify native", flag.ContinueOnError)
		...
		if err := flags.Parse(args[1:]); err != nil || flags.NArg() != 0 || !supportedNativePlatform(*platform) {
			return errors.New("usage: civerify native --platform <darwin|linux|windows>")
		}
	...
	case "trust-input":
		flags := flag.NewFlagSet("civerify trust-input", flag.ContinueOnError)
		flags.SetOutput(io.Discard)
		...
		if err := flags.Parse(args[1:]); err != nil || flags.NArg() != 0 || *output == "" || *githubEnvironment == "" {
			return errors.New("usage: civerify trust-input --output <path> --github-env <path>")
		}
		return writeTrustInput(*output, *githubEnvironment)
	case "fetch-tags":
		if len(args) != 1 {
			return errors.New("usage: civerify fetch-tags")
		}
		return fetchTags(runner)
	default:
		return fmt.Errorf("unknown civerify command: %s", args[0])
	}
}
