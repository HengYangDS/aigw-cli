// CI projection contracts share this command's single composition root.
package main
...
func runContract(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: ci <toolchain|proxy-policy|github-verify|github-release|pipeline> [root]")
	}
	root := "."
	if len(args) > 1 {
		root = args[1]
