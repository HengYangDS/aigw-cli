...
func TestRunSBOMUsesRealModuleMetadata(t *testing.T) {
	...
	if namespace, ok := document["documentNamespace"].(string); !ok || !strings.HasPrefix(namespace, "urn:sha256:") || len(namespace) != len("urn:sha256:")+64 {
		t.Fatalf("document namespace = %#v", document["documentNamespace"])
	}
...
func TestDocumentNamespaceTracksTheCanonicalModuleGraph(t *testing.T) {
	created := time.Unix(1, 0).UTC()
	left := []module{{Path: "example.test/b", Version: "v2"}, {Path: "example.test/a", Version: "v1"}}
	right := []module{{Path: "example.test/a", Version: "v1"}, {Path: "example.test/b", Version: "v2"}}
	if documentNamespace("1.0.0", created, left) != documentNamespace("1.0.0", created, right) {
		t.Fatal("document namespace depends on module enumeration order")
	}
	right[1].Version = "v3"
	if documentNamespace("1.0.0", created, left) == documentNamespace("1.0.0", created, right) {
		t.Fatal("document namespace ignores module graph changes")
	}
}
