...
// ValidateVersion checks whether the release version has sufficient native
// signing evidence for its stability level.
...
var staleEvidence = regexp.MustCompile(`Current status \(20[0-9]{2}-[0-9]{2}-[0-9]{2}\)|0\.1\.0-rc\.[0-9]+|codex/initial-product|GitLab SSH|GitLab API|e082b00`)

// ValidateDocument rejects release evidence that embeds a historical snapshot.
func ValidateDocument(path string) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return fmt.Errorf("cannot read release evidence contract: %w", err)
	}
	if staleEvidence.Match(data) {
