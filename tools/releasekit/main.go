...
func run(args []string, stdout io.Writer) error {
	...
	case "build-ci":
		if len(args) != 3 {
			return errors.New("usage: releasekit build-ci <workspace> <output-directory>")
		}
		root, err := os.Getwd()
		if err != nil {
			return err
	...
	case "validate-toolchain":
		if len(args) != 2 {
			return errors.New("usage: releasekit validate-toolchain <go.mod>")
		}
		return validateToolchain(args[1], runtime.Version())
	case "validate-readiness":
		if len(args) != 2 {
			return errors.New("usage: releasekit validate-readiness <version>")
		}
		return validateReleaseReadiness(args[1])
	case "validate-readiness-tag":
		if len(args) != 1 {
			return errors.New("usage: releasekit validate-readiness-tag")
		}
	...
	case "validate-readiness-doc":
		if len(args) != 2 {
			return errors.New("usage: releasekit validate-readiness-doc <document>")
		}
		return validateReleaseReadinessDocument(args[1])
	case "validate-artifacts":
		if len(args) != 3 {
			return errors.New("usage: releasekit validate-artifacts <directory> <version>")
		}
		return validateArtifactMatrix(args[1], args[2])
	case "compare-artifacts":
		if len(args) != 4 {
			return errors.New("usage: releasekit compare-artifacts <left-directory> <right-directory> <version>")
		}
		return compareArtifactMatrices(args[1], args[2], args[3])
	case "publish-github":
		if len(args) != 2 {
			return errors.New("usage: releasekit publish-github <artifact-directory>")
		}
	...
	case "upload-gitlab":
		if len(args) != 2 {
			return errors.New("usage: releasekit upload-gitlab <artifact-directory>")
		}
	...
	case "publish-gitlab":
		if len(args) != 2 {
			return errors.New("usage: releasekit publish-gitlab <artifact-directory>")
		}
