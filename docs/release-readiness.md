...
| Windows RC assurance | A blocking managed Windows-runner test, build, installer, execution, PATH, and shim proof plus MSI/ZIP layout and architecture checks | Cross-compilation alone, MSI metadata alone, or an allowed failure |
| Windows native runtime works | Blocking managed Windows-runner package, install, upgrade, uninstall, PATH, shim, and execution evidence | Non-Windows PowerShell syntax or structural package checks |
