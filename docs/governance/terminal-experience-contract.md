...
An `orphaned-aigw-marker` on Air has no recognized sidecar binding. It is a
diagnostic boundary, not proof that AIGW can safely remove the marked text.
Route doctor therefore states that no AIGW mutation is admitted and recommends
another read-only ownership report; it must not suggest generic repair or Air
recovery for that state.
