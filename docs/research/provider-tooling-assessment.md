...
| Hermes Agent                      | Hermes documents provider/model configuration, environment credential references, native Windows, macOS Apple Silicon, and Linux support.                                     | Integrate its own provider contract and preserve its sessions and services. The inspected native `key_cmd` reader and callable token source provide the credential path; a plugin authentication type is a different contract. |
