...
packager="$root/scripts/package-verified-candidate.sh"
checker="$root/scripts/check-verified-candidate.sh"
...
sh "$root/scripts/check-release-artifacts.sh" "$dist" "$version" >/dev/null
...
cp "$packager" "$checker" "$clone/scripts/"
chmod 755 "$clone/scripts/package-verified-candidate.sh" "$clone/scripts/check-verified-candidate.sh"
(cd "$clone" && sh scripts/package-verified-candidate.sh "$version" "$dist" "$out") >/dev/null
...
sh "$clone/scripts/check-verified-candidate.sh" "$carrier" >/dev/null
...
if sh "$clone/scripts/check-verified-candidate.sh" "$tampered" >/dev/null 2>&1; then
...
PY
if sh "$clone/scripts/check-verified-candidate.sh" "$unsafe" >/dev/null 2>&1; then
