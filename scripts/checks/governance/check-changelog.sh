...
python3 - "$changelog" "$latest_tag" "$selected_tag" "$root/.config/release/retired-gitlab-tags.txt" "$forge" <<'PYTHON'
...
if selected_tag:
    ...
    latest_version = managed_versions[0]
    if not unknown:
    ...
    elif release_key(unknown[0]) <= release_key(managed_versions[0]):
