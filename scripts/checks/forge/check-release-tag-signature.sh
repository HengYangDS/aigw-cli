...
case "$provider" in
  gitlab) allowed_signers=${AIGW_GITLAB_ALLOWED_SIGNERS:-} ;;
  github) allowed_signers=${AIGW_GITHUB_ALLOWED_SIGNERS:-} ;;
  *) echo "release tag provider must be gitlab or github: $provider" >&2; exit 2 ;;
esac
[ -n "$allowed_signers" ] || {
  echo "$provider release-tag trust input is required" >&2
  exit 2
}
...
test -f "$allowed_signers" || {
  echo "release tag trust input is missing: $allowed_signers" >&2
  exit 1
}
