...
acceptance="$root/scripts/tests/install/test-macos-native-install.sh"
...
if ! grep -Fq "find \"\$tree\" -type f -name '._*' -delete" "$root/scripts/release/build/package.sh"; then
