...
env \
  ...
  /bin/sh "$root/scripts/install/install.sh"
...
set +e
env -i HOME="$home" PATH="" AIGW_INSTALL_DIR="$install_dir" /bin/sh "$root/scripts/install/uninstall.sh" --help >"$tmp/uninstall-help.out" 2>"$tmp/uninstall-help.err"
uninstall_help_rc=$?
set -e
...
env -i HOME="$home" PATH="" AIGW_INSTALL_DIR="$install_dir" /bin/sh "$root/scripts/install/uninstall.sh"
