...
expect_timeout_upper_bound_rejected() {
  ...
  if PATH="$bin:/usr/bin:/bin" \
    ...
    sh "$root/scripts/tests/install/test-linux-native-install.sh" "$out" "$version" >"$output" 2>&1
  then
...
PATH="$bin:/usr/bin:/bin" \
  ...
  AIGW_LINUX_DEB_ACCEPTANCE_IMAGE="example.test/debian" \
  AIGW_LINUX_RPM_ACCEPTANCE_IMAGE="example.test/rpm" \
  sh "$root/scripts/tests/install/test-linux-native-install.sh" "$out" "$version" >/dev/null
...
start_supervisor_case() {
  ...
  PATH="$supervisor_bin" \
    ...
    sh "$root/scripts/tests/install/test-linux-native-install.sh" "$out" "$version" >"$supervisor_output" 2>&1 &
...
if PATH="$lock_bin:$PATH" \
  ...
  AIGW_LINUX_DEB_ACCEPTANCE_IMAGE="example.test/debian" \
  AIGW_LINUX_RPM_ACCEPTANCE_IMAGE="example.test/rpm" \
  sh "$root/scripts/tests/install/test-linux-native-install.sh" "$out" "$version" >"$tmp/lock.out" 2>&1; then
...
PATH="$lock_bin:$PATH" \
  ...
  AIGW_LINUX_DEB_ACCEPTANCE_IMAGE="example.test/debian" \
  AIGW_LINUX_RPM_ACCEPTANCE_IMAGE="example.test/rpm" \
  sh "$root/scripts/tests/install/test-linux-native-install.sh" "$out" "$version" >"$tmp/lock-cancel.out" 2>&1 &
lock_cancel_pid=$!
sleep 1
