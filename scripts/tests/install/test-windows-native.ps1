...
try {
    ...
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $Repository "scripts/tests/install/test-installers.ps1") -Installer (Join-Path $Repository "scripts/install/install.ps1")
    if ($LASTEXITCODE -ne 0) { throw "portable Windows installer harness failed" }

    & go test ./internal/claude -run '^TestLauncherCreatesWindowsCommandThatCanRunAIGW$' -count=1
    if ($LASTEXITCODE -ne 0) { throw "Windows Claude launcher execution contract failed" }
