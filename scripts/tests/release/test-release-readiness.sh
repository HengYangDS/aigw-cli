...
if ! sh "$root/scripts/checks/release/check-release-readiness.sh" 0.1.0-rc.1 >/dev/null; then
  echo "release candidate readiness must remain packageable" >&2
  exit 1
fi

if sh "$root/scripts/checks/release/check-release-readiness.sh" 0.1.0 >/dev/null 2>&1; then
  echo "unsigned GA release must fail closed" >&2
  exit 1
fi

if ! sh "$root/scripts/checks/release/check-release-readiness-doc.sh" "$root/docs/operations/release-readiness.md" >/dev/null; then
...
EOF
if sh "$root/scripts/checks/release/check-release-readiness-doc.sh" "$scratch/stale.md" >"$scratch/out" 2>"$scratch/err"; then
...
if sh "$root/scripts/checks/release/check-release-readiness-doc.sh" "$scratch/missing.md" >"$scratch/out" 2>"$scratch/err"; then
