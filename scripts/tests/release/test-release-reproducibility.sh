...
build() {
  ...
  env \
    ...
    AIGW_GITLAB_RELEASE_ORIGIN="${AIGW_GITLAB_RELEASE_ORIGIN:-https://gitlab.example}" \
    AIGW_GITLAB_RELEASE_REPOSITORY="${AIGW_GITLAB_RELEASE_REPOSITORY:-group/aigw-cli}" \
    AIGW_GITHUB_RELEASE_ORIGIN="${AIGW_GITHUB_RELEASE_ORIGIN:-https://github.example}" \
    AIGW_GITHUB_RELEASE_REPOSITORY="${AIGW_GITHUB_RELEASE_REPOSITORY:-organization/aigw-cli}" \
    AIGW_PACKAGE_HOMEPAGE="${AIGW_PACKAGE_HOMEPAGE:-https://product.example/aigw-cli}" \
    sh "$root/scripts/release/build/package.sh" "$version" "$out" >/dev/null
  sh "$root/scripts/tests/release/test-release-package-layout.sh" "$out" "$version" >/dev/null
}
