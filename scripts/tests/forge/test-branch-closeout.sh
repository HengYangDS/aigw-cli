...
checker="$root/scripts/checks/forge/check-branch-closeout.sh"
tmp=$(mktemp -d "${TMPDIR:-/tmp}/aigw-branch-closeout.XXXXXX")
trap 'rm -rf "$tmp"' EXIT HUP INT TERM

source="$tmp/source"
git -c core.fsmonitor=false init -q -b main "$source"
git -c core.fsmonitor=false -C "$source" config core.hooksPath /dev/null
...
git -c core.fsmonitor=false -C "$projection" config core.hooksPath /dev/null
