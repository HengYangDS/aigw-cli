...
cp "$root/scripts/checks/governance/check-text-layout.py" "$tmp/check.py"
...
git add README.md
mkdir -p "$tmp/hooks"
commit_output=$(git -c core.hooksPath="$tmp/hooks" commit -qm initial 2>&1) || {
  printf '%s\n' "$commit_output" >&2
  exit 1
}
if [ -n "$commit_output" ]; then
  printf 'fixture commit emitted unexpected output:\n%s\n' "$commit_output" >&2
  exit 1
fi
...
env \
  ...
  python3 scripts/checks/governance/check-text-layout.py >/dev/null
printf '# Invalid\n\n\nDouble gap.\n' > README.md
if python3 scripts/checks/governance/check-text-layout.py >/dev/null 2>&1; then
