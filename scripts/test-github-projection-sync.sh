...
# The production synchronizer verifies only existing provider provenance tags.
# No tag is needed for the isolation regression: CI must remain portable and
# must not require an operator's private SSH signing key.
