...
"$root/scripts/check-release-artifacts.sh" "$dist_abs" "$version" >/dev/null
...
for script in check-release-artifacts.sh check-verified-candidate.sh; do
  cp "$root/scripts/$script" "$payload/$script"
  chmod 755 "$payload/$script"
done
