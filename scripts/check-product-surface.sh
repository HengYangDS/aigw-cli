...
grep -Fq 'maintainer: Yang HENG <heng.yang.ds@hotmail.com>' packaging/linux/nfpm.yaml || fail "Linux package metadata must declare the canonical maintainer"
grep -Fq 'vendor: AIGW CLI' packaging/linux/nfpm.yaml || fail "Linux package metadata must declare the formal product vendor"
grep -Fq 'homepage: https://github.com/HengYangDS/aigw-cli' packaging/linux/nfpm.yaml || fail "Linux package metadata must declare the canonical project homepage"
