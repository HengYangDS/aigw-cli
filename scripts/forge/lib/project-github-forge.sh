...
[ -n "$github_allowed_signers" ] || { echo "GitHub trust input is required through AIGW_GITHUB_ALLOWED_SIGNERS" >&2; exit 2; }
[ -n "$gitlab_allowed_signers" ] || { echo "GitLab trust input is required through AIGW_GITLAB_ALLOWED_SIGNERS" >&2; exit 2; }
...
AIGW_GITLAB_ALLOWED_SIGNERS="$gitlab_allowed_signers" \
  ...
  sh "$script_root/scripts/checks/forge/check-commit-provenance.sh" "$projection" gitlab >/dev/null
