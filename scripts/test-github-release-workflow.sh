...
required = [
    ...
    'scripts/check-release-tag-signature.sh . "$SELECTED_TAG" github', 'AIGW_CHANGELOG_RELEASE_TAG: ${{ inputs.tag || github.ref_name }}', 'git rev-parse "$SELECTED_TAG^{}"', "scripts/check-release-toolchain.sh", "scripts/check-static-analysis.sh", "AIGW_REQUIRE_FULL_MATRIX=1 sh scripts/package.sh",
