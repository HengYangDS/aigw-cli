...
"$root/scripts/compare-release-artifacts.sh" "$left" "$right" "$version" >/dev/null
printf 'different\n' >> "$right/aigw_${version}_windows_arm64.zip"
(cd "$right" && shasum -a 256 $names > checksums.txt)
if "$root/scripts/compare-release-artifacts.sh" "$left" "$right" "$version" >"$tmp/different.out" 2>&1; then
