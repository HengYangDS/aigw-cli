	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.file.closes != 1 {
				t.Fatalf("temporary handle close count = %d, want one", test.file.closes)
			}
		})
	}
}

func TestCommitTemporaryFilePreservesWriteAndCloseFailures(t *testing.T) {
	writeErr, closeErr := errors.New("write failed"), errors.New("close failed")
	file := &failingTemporaryFile{writeErr: writeErr, closeErr: closeErr}
	err := commitTemporaryFile(file, "target", []byte("data"), 0o600)
	if !errors.Is(err, writeErr) || !errors.Is(err, closeErr) || file.closes != 1 {
		t.Fatalf("write and close outcomes were lost: closes=%d, %v", file.closes, err)
	}
}

func TestAtomicWriteRemovesOnlyItsFailedStagingFile(t *testing.T) {
	root := t.TempDir()
	target, foreign := filepath.Join(root, "target"), filepath.Join(root, "operator-file")
	if err := os.Mkdir(target, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(foreign, []byte("preserved"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := WriteFileAtomic(target, []byte("replacement"), 0o600); err == nil {
		t.Fatal("file unexpectedly replaced the target directory")
	}
	entries, err := os.ReadDir(root)
	if err != nil || len(entries) != 2 {
		t.Fatalf("failed write left temporary files: %v, %v", entries, err)
	}
	if data, err := os.ReadFile(foreign); err != nil || string(data) != "preserved" {
		t.Fatalf("failed write changed foreign file: %q, %v", data, err)
	}
}
