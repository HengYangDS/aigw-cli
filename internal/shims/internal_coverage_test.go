...
func TestValidateShimTargetReportsFilesystemErrors(t *testing.T) {
	t.Run("symbolic link loop", func(t *testing.T) {
		loop := filepath.Join(t.TempDir(), "loop")
		if err := os.Symlink(loop, loop); err != nil {
			t.Fatal(err)
		}
		ready, err := validateShimTarget(loop, false)
		if err == nil || ready || !strings.Contains(err.Error(), "inspect AIGW-managed Claude shim target") {
			t.Fatalf("validateShimTarget() = %v, %v", ready, err)
		}
	})

	t.Run("directory", func(t *testing.T) {
		ready, err := validateShimTarget(t.TempDir(), false)
...
func TestUnixShimTargetParsing(t *testing.T) {
	tests := []struct {
		name    string
		content string
		target  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target, ok := unixShimTarget(tt.content)
			if target != tt.target || ok != tt.ok {
				t.Fatalf("unixShimTarget() = %q, %v; want %q, %v", target, ok, tt.target, tt.ok)
			}
		})
	}
}

func TestWindowsShimTargetParsing(t *testing.T) {
	tests := []struct {
		name    string
		content string
		target  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target, ok := windowsShimTarget(tt.content)
			if target != tt.target || ok != tt.ok {
				t.Fatalf("windowsShimTarget() = %q, %v; want %q, %v", target, ok, tt.target, tt.ok)
			}
		})
	}
}
...
func TestRestoreClaudeStateRejectsChangedPaths(t *testing.T) {
	manager := Manager{GOOS: "linux", BinDir: t.TempDir(), Home: t.TempDir(), Shell: "/bin/sh"}
	before, err := manager.CaptureClaudeState()
	if err != nil {
		t.Fatal(err)
	}

	differentLauncher := Manager{GOOS: "linux", BinDir: t.TempDir(), Home: manager.Home, Shell: manager.Shell}
	...
	differentActivation := Manager{GOOS: "linux", BinDir: manager.BinDir, Home: t.TempDir(), Shell: manager.Shell}
...
func TestRestoreClaudeStateRejectsConcurrentEditsToBothFiles(t *testing.T) {
	binDir := t.TempDir()
	home := t.TempDir()
	manager := Manager{GOOS: "linux", BinDir: binDir, Home: home, Shell: "/bin/sh"}
...
func TestRollbackClaudeEnableReportsCaptureAndRestoreErrors(t *testing.T) {
	...
	t.Run("capture postimage", func(t *testing.T) {
		manager := Manager{GOOS: "linux", BinDir: t.TempDir()}
		before, err := manager.CaptureClaudeState()
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("restore path guard", func(t *testing.T) {
		manager := Manager{GOOS: "linux", BinDir: t.TempDir()}
		other := Manager{GOOS: "linux", BinDir: t.TempDir()}
