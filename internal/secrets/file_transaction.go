...
func replaceSecureFile(root writeRoot, name string, value []byte) (_ credentialFileSnapshot, committed bool, result error) {
	...
	defer func() {
		...
		if !committed {
			if err := root.Remove(temporaryName); err != nil && !errors.Is(err, os.ErrNotExist) {
				result = errors.Join(result, fmt.Errorf("remove Token staging file %q: %w", temporaryName, err))
			}
		}
	}()
...
func restoreSecureFile(root writeRoot, name string, preimage, postimage credentialFileSnapshot, cause error) error {
	current, err := captureOptionalSecureFile(root, name)
	if err != nil {
		return fmt.Errorf("%w; inspect Token postimage before compensation: %w", cause, err)
	}
	defer clear(current.value)
	...
	if !preimage.exists {
		if err := root.Remove(name); err != nil && !errors.Is(err, os.ErrNotExist) {
			return fmt.Errorf("%w; remove uncommitted Token: %w", cause, err)
		}
		if err := syncRoot(root); err != nil {
			return fmt.Errorf("%w; sync restored Token state: %w", cause, err)
		}
		return cause
	}
	...
	if err != nil {
		return fmt.Errorf("%w; restore previous Token: %w", cause, err)
	}
	return cause
}
