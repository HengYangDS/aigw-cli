...
func TestUpdateRejectsPeerIntegrityFailureBeforeReplacingBinary(t *testing.T) {
	...
	runner := &releaseRunner{archive: gitlabArchive, checksum: strings.Repeat("0", 64) + "  " + archiveName + "\n"}
	u := upgrade.Updater{
		GOOS:       "darwin",
		GOARCH:     "arm64",
		Executable: binary,
...
func TestUpdateRejectsReachablePeerAuthorizationFailure(t *testing.T) {
	...
	t.Setenv("AIGW_GITHUB_RELEASE_ORIGIN", github.URL)
	t.Setenv("AIGW_GITHUB_RELEASE_REPOSITORY", "example-owner/aigw-cli")
	runner := &releaseRunner{archive: []byte("unused"), checksum: "unused"}
	...
	u := upgrade.Updater{
		GOOS: "darwin", GOARCH: "arm64", Executable: binary, Runner: &unavailableRunner{}, HTTPClient: client,
		GitLab: upgrade.ReleaseSource{Provider: upgrade.ReleaseProviderGitLab, Origin: "https://gitlab.example.test", Repository: testReleaseProject},
	}
...
func TestUpdateUsesReachablePeerWhenOtherPeerIsUnavailable(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"environment source", "", upgrade.Updater{Runner: &unavailableRunner{}}},
		{"authenticated API source", "test-token", upgrade.Updater{Runner: &unavailableRunner{}, GitLab: gitlab}},
		{"unavailable peer workspace", "", upgrade.Updater{Runner: &releaseRunner{}, GitLab: gitlab}},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Setenv("AIGW_GITLAB_RELEASE_ORIGIN", gitlab.Origin)
			t.Setenv("AIGW_GITLAB_RELEASE_REPOSITORY", gitlab.Repository)
			t.Setenv("AIGW_GITHUB_RELEASE_ORIGIN", github.URL)
			t.Setenv("AIGW_GITHUB_RELEASE_REPOSITORY", "example-owner/aigw-cli")
			t.Setenv("GITLAB_TOKEN", test.token)
			test.updater.GOOS, test.updater.GOARCH, test.updater.Executable = "darwin", "arm64", binary
			test.updater.HTTPClient = &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
			...
			for _, name := range []string{"TMPDIR", "TEMP", "TMP"} {
				t.Setenv(name, updateTempRoot)
			}
			message, err := test.updater.Update(context.Background(), "0.1.0")
			if err != nil {
				t.Fatal(err)
			}
			if got, err := os.ReadFile(binary); err != nil || string(got) != "github-binary" {
				t.Fatalf("installed binary = %q, %v", got, err)
			}
			if !strings.Contains(message, "github") || strings.Contains(message, "gitlab and github") {
				t.Fatalf("message = %q", message)
			}
			if after, err := os.ReadDir(updateTempRoot); err != nil || len(after) != 0 {
				t.Fatalf("update workspace remains: %v, %v", after, err)
			}
		})
	}
}
...
func TestUpdateRejectsPeerAssetDisagreementBeforeReplacingBinary(t *testing.T) {
	...
	t.Setenv("AIGW_GITHUB_RELEASE_ORIGIN", github.URL)
	t.Setenv("AIGW_GITHUB_RELEASE_REPOSITORY", "example-owner/aigw-cli")
	runner := &releaseRunner{archive: gitlabArchive, checksum: fmt.Sprintf("%x  %s\n", sha256.Sum256(gitlabArchive), archiveName)}
	u := upgrade.Updater{
		GOOS:       "darwin",
		GOARCH:     "arm64",
		Executable: binary,
