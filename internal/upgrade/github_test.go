...
func TestLatestPrereleaseTagFromGitHubReturnsOriginalErrorWhenNotNotFound(t *testing.T) {
	...
	if _, err := u.latestPrereleaseTagFromGitHub(context.Background(), ReleaseSource{}, original); !errors.Is(err, original) {
...
func TestLatestPrereleaseTagFromGitHubReturnsOriginalErrorOnOtherFailureStatus(t *testing.T) {
	...
	u := Updater{HTTPClient: server.Client()}
	source := ReleaseSource{Origin: server.URL, Repository: "o/r"}
	if _, err := u.latestPrereleaseTagFromGitHub(context.Background(), source, original); !errors.Is(err, original) {
		t.Fatalf("error = %v, want %v", err, original)
	}
}
...
func TestLatestTagFromGitHubReleaseReturnsPrereleaseErrorWhenCLIFallbackFails(t *testing.T) {
	...
	defer server.Close()
	u := Updater{HTTPClient: server.Client(), Runner: &recordingRunner{err: errors.New("gh missing")}}
	source := ReleaseSource{Origin: server.URL, Repository: "o/r"}
	_, err := u.latestTagFromGitHubRelease(context.Background(), source)
...
func TestAuthorizeGitHubRequestSelectsConfiguredToken(t *testing.T) {
	for _, test := range []struct {
		name, aigw, github, gh, want string
	}{
		{"AIGW token", "aigw-token", "github-token", "gh-token", "Bearer aigw-token"},
		{"GitHub token", "", "github-token", "gh-token", "Bearer github-token"},
		{"GH token", "", "", "gh-token", "Bearer gh-token"},
		{"no token", "", "", "", ""},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Setenv("AIGW_GITHUB_TOKEN", test.aigw)
			...
			if err := (&Updater{}).authorizeGitHubRequest(request); err != nil {
				t.Fatal(err)
			}
...
func (r *githubCLIRunner) RunCapture(_ context.Context, plan process.Plan) ([]byte, error) {
	args := plan.Args
...
func (r *writingCommandRunner) RunCapture(_ context.Context, plan process.Plan) ([]byte, error) {
	if plan.Executable == "gh" {
