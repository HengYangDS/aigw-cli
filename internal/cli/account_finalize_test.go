...
func TestAccountFinalizeRequiresCurrentFullVerificationCheckpoint(t *testing.T) {
	...
	t.Run("stale checkpoint", func(t *testing.T) {
		...
		if err := app.Config.SaveVerifiedCheckpoint(before, domain.AdmittedClientIDs()); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("partial client coverage", func(t *testing.T) {
		app, _, secretStore, current := prepareAccountFinalizer(t, []string{domain.ClientCodex})
		if err := secretStore.Set("zeta-new", "target-token"); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("source account remains", func(t *testing.T) {
		...
		if err := app.Config.SaveVerifiedCheckpoint(current, domain.AdmittedClientIDs()); err != nil {
			t.Fatal(err)
		}
		if err := secretStore.Set("zeta-new", "target-token"); err != nil {
			t.Fatal(err)
...
func TestAccountFinalizeDryRunApplyAndAlreadyFinalized(t *testing.T) {
	app, out, secretStore, current := prepareAccountFinalizer(t, domain.AdmittedClientIDs())
	...
	if !secretStore.Has("zeta-old") || !app.Accounts.Has("zeta-old") {
...
func TestAccountFinalizeRequiresTargetAPIToken(t *testing.T) {
	app, _, secretStore, _ := prepareAccountFinalizer(t, domain.AdmittedClientIDs())
	...
	if !bytes.Equal(readFile(t, app.Config.Path()+".bak"), backupBefore) || !secretStore.Has("zeta-old") {
...
func TestAccountFinalizeCredentialRotationRequiresConfirmationAndLiveProbe(t *testing.T) {
	...
	providerAccount.AccountProbe = &domain.AccountProbe{Kind: "dmxapi", BaseURL: "https://dmx.test"}
	...
	if requests != 0 || !secretStore.Has("zeta-old") || !app.Accounts.Has("zeta-old") {
	...
	if requests != 2 || secretStore.Has("zeta-old") || app.Accounts.Has("zeta-old") {
		t.Fatalf("confirmed rotation requests=%d source_token=%v source_probe=%v", requests, secretStore.Has("zeta-old"), app.Accounts.Has("zeta-old"))
	}
...
func TestAccountFinalizePartialDeleteCanRetryAfterBackupConvergence(t *testing.T) {
	...
	if secretStore.Has("zeta-old") || !baseAccounts.Has("zeta-old") {
		t.Fatalf("partial delete state: source_token=%v source_probe=%v", secretStore.Has("zeta-old"), baseAccounts.Has("zeta-old"))
	}
...
func TestAccountFinalizeEnvironmentCleanupIsExternalAndRetryable(t *testing.T) {
	app, _, _, _ := prepareAccountFinalizer(t, domain.AdmittedClientIDs())
...
func prepareAccountFinalizer(t *testing.T, clients []string) (*cli.App, *bytes.Buffer, *secrets.MemoryStore, domain.Config) {
	t.Helper()
	app, out, secretStore, _ := testApp(t, "")
	before := accountRenameConfig()
	if err := app.Config.Save(before); err != nil {
...
func renamedAccountConfig(cfg domain.Config) domain.Config {
...
func assertCurrentAccountConfig(t *testing.T, app *cli.App, want domain.Config) {
	t.Helper()
