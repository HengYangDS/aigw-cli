...
func NewCommand(deps Dependencies) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			renderClients(r, result.Clients)
			r.Section("Checks")
...
func collectResult(ctx context.Context, deps Dependencies) commandResult {
	backend, backendErr := secrets.Inspect(deps.Secrets)
	checks := Collect(ctx, deps)
	if backendErr != nil {
		checks = append([]Check{{
	...
	for _, spec := range configuration.AdmittedClientSpecs() {
		client, observed := result.Clients[spec.ID]
		if !observed {
			continue
		}
		switch client.State {
		...
		default:
			result.OK = false
			client.NextAction = "aigw doctor --json"
		}
		if result.NextAction == "" {
			result.NextAction = client.NextAction
			if result.NextAction == "" {
...
func Detail(check Check) string {
	...
	case name == "adapter:claude" || name == "adapter:codex":
		...
		if strings.Contains(detail, "executable is not configured") {
			return "Enabled, but no executable is configured"
		}
		if detail == "Codex configuration target is missing" {
