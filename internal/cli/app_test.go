...
func TestDefaultShimDirectoryIsUserBinNotExecutableDirectory(t *testing.T) {
	...
	want, err := platform.UserBinDirFor("darwin", env)
	if err != nil {
		t.Fatal(err)
	}
	if got != want || got == "/usr/local/bin" {
		t.Fatalf("shim dir = %q, want %q and not executable dir", got, want)
	}
}
