...
func TestAccountRenameNonCurrentCodexAccountDoesNotReauthenticate(t *testing.T) {
	...
	cfg := accountRenameConfig()
	cfg.Accounts["active"] = domain.Account{Label: "Active", Endpoints: domain.Endpoints{OpenAIResponses: "https://active.test/v1"}}
	cfg.Profiles["active-profile"] = domain.Profile{Label: "Active", Account: "active", Client: domain.ClientCodex, Models: domain.Models{domain.ClientCodex: "active-model"}}
	cfg.Routes.Default = "active-profile"
	cfg.Adapters[domain.ClientCodex] = domain.AdapterConfig{Enabled: true, Executable: "/opt/codex", Targets: []string{filepath.Join(t.TempDir(), "codex", "config.toml")}}
	if err := app.Config.Save(cfg); err != nil {
		t.Fatal(err)
	}
...
func accountRenameConfig() domain.Config {
	cfg := domain.NewConfig()
	cfg.Accounts["alpha"] = domain.Account{Label: "Alpha", Endpoints: domain.Endpoints{OpenAIResponses: "https://alpha.test/v1"}}
	cfg.Accounts["zeta-old"] = domain.Account{
		Label: "Zeta",
		Endpoints: domain.Endpoints{
			OpenAIResponses: "https://zeta.test/v1",
			Anthropic:       "https://zeta.test/anthropic",
		},
		AccountProbe: &domain.AccountProbe{Kind: "future-provider", BaseURL: "https://probe.zeta.test"},
	}
	cfg.Profiles["codex-profile"] = domain.Profile{Label: "Codex", Purpose: "Codex purpose", Account: "zeta-old", Client: domain.ClientCodex, Models: domain.Models{domain.ClientCodex: "codex-model"}}
	cfg.Profiles["claude-profile"] = domain.Profile{Label: "Claude", Purpose: "Claude purpose", Account: "zeta-old", Client: domain.ClientClaude, Models: domain.Models{domain.ClientClaude: "claude-model"}}
	cfg.Routes.Default = "codex-profile"
	cfg.Routes.Overrides[domain.ClientClaude] = "claude-profile"
	return cfg
}
