...
func TestProfileRenameInteractiveZeroArgsSortsChoicesAndUpdatesRoutes(t *testing.T) {
	...
	cfg.Profiles["alpha"] = domain.Profile{Label: "Alpha", Account: "gateway", Client: domain.ClientCodex, Models: domain.Models{domain.ClientCodex: "alpha"}}
	cfg.Routes.Overrides[domain.ClientCodex] = "zeta-old"
	...
	if got.Routes.Default != "zeta-new" || got.Routes.Overrides[domain.ClientCodex] != "zeta-new" {
...
func TestProfileRenameRefusesInvalidOrConflictingTargetWithoutMutation(t *testing.T) {
	for _, target := range []string{"alpha", "Invalid Target"} {
		t.Run(target, func(t *testing.T) {
			app, _, _, _ := testApp(t, "")
			cfg := profileRenameConfig()
			cfg.Profiles["alpha"] = domain.Profile{Label: "Alpha", Account: "gateway"}
			if err := app.Config.Save(cfg); err != nil {
				t.Fatal(err)
			}
			before, err := os.ReadFile(app.Config.Path())
...
func profileRenameConfig() domain.Config {
	cfg := domain.NewConfig()
	cfg.Accounts["gateway"] = domain.Account{Label: "Gateway", Endpoints: domain.Endpoints{OpenAIResponses: "https://gateway.test/v1"}}
	cfg.Profiles["zeta-old"] = domain.Profile{Label: "Zeta", Purpose: "Keep this label and purpose", Account: "gateway", Client: domain.ClientCodex, Models: domain.Models{domain.ClientCodex: "zeta-model"}}
	cfg.Routes.Default = "zeta-old"
	return cfg
}

func choiceValues(choices []cli.Choice) []string {
