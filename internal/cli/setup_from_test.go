...
const teamSetupManifest = `version = 3
...
type teamSetupPrompt struct {
	secrets     []string
	secretCalls []string
	textCalls   int
}

func (p *teamSetupPrompt) Secret(label string) (string, error) {
...
func (p *teamSetupPrompt) Text(string) (string, error) {
	p.textCalls++
	return "", errors.New("unexpected text prompt")
}

func (p *teamSetupPrompt) Select(string, []cli.Choice) (string, error) {
	return "", errors.New("unexpected select prompt")
}

func writeTeamSetupManifest(t *testing.T, body string) string {
	t.Helper()
	path := filepath.Join(t.TempDir(), "team-profiles.toml")
...
func assertTeamSetupLeavesNoConfig(t *testing.T, app *cli.App) {
	t.Helper()
...
func TestSetupFromTeamManifestPromptsOnlyForTokensAndKeepsThemSecret(t *testing.T) {
	t.Setenv("AIGW_TOKEN_UNRELATED", "aigw-test-unrelated-token")
	app, out, secretStore, runner := testApp(t, "")
	app.Interactive = true
	prompt := &teamSetupPrompt{secrets: []string{"aigw-test-aihubmix-token", "aigw-test-dmxapi-token"}}
	app.Prompt = prompt
	shimDir := t.TempDir()
	...
	manifestPath := writeTeamSetupManifest(t, teamSetupManifest)
	...
	if cfg.Profiles["aihubmix-claude"].Models["claude"] != "claude-test" || cfg.Profiles["dmxapi-gpt"].Models["codex"] != "gpt-test" {
		t.Fatalf("team model matrix was not preserved: %#v", cfg.Profiles)
	}
...
func TestSetupFromTeamManifestReusesEnvironmentTokensWithoutPrompting(t *testing.T) {
	app, _, _, _ := testApp(t, "")
...
func TestSetupFromTeamManifestRejectsCredentialsBeforePromptOrWrite(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	prompt := &teamSetupPrompt{secrets: []string{"must-not-be-read"}}
	app.Prompt = prompt
	manifestPath := writeTeamSetupManifest(t, strings.Replace(teamSetupManifest, "label = \"AIHubMix\"", "label = \"AIHubMix\"\ntoken = \"forbidden\"", 1))
	...
	assertTeamSetupLeavesNoConfig(t, app)
}

func TestSetupFromTeamManifestNonInteractiveListsMissingTokensWithoutWrite(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	manifestPath := writeTeamSetupManifest(t, teamSetupManifest)
...
func TestSetupFromTeamManifestRejectsOneStdinTokenForMultipleAccounts(t *testing.T) {
...
func TestSetupFromTeamManifestRejectsExplicitEmptySingleProfileFlag(t *testing.T) {
	app, _, _, _ := testApp(t, "")
	manifestPath := writeTeamSetupManifest(t, teamSetupManifest)
...
func TestSetupFromTeamManifestRefusesMultipleCodexTargetsBeforePromptOrWrite(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	prompt := &teamSetupPrompt{secrets: []string{"must-not-be-read"}}
	app.Prompt = prompt
	app.Discovery = fakeDiscovery{result: discovery.Result{
		CodexExecutable: "/opt/codex-real",
		Surfaces: []discovery.Surface{
			{ID: discovery.SurfaceCodexCLIStandalone, Authority: discovery.AuthorityAIGW, ConfigPath: filepath.Join(t.TempDir(), "one", "config.toml"), Present: true, AutoManaged: true},
			{ID: "second-codex", Authority: discovery.AuthorityAIGW, ConfigPath: filepath.Join(t.TempDir(), "two", "config.toml"), Present: true, AutoManaged: true},
		},
	}}
	manifestPath := writeTeamSetupManifest(t, teamSetupManifest)
...
func TestSetupFromTeamManifestValidationFailureLeavesNoCredentialsOrConfig(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	app.Prompt = &teamSetupPrompt{secrets: []string{"aigw-test-aihubmix-token", "aigw-test-dmxapi-token"}}
	requests := 0
	app.HTTP = &fakeHTTP{handler: func(req *http.Request) (*http.Response, error) {
		requests++
...
func TestSetupFromTeamManifestRejectsClaudeNonSuccessAsUnvalidated(t *testing.T) {
...
func TestSetupFromTeamManifestDoesNotFollowCredentialProbeRedirects(t *testing.T) {
	...
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	app.Prompt = &teamSetupPrompt{secrets: []string{"aigw-test-team-token"}}
	app.HTTP = &http.Client{}
	manifestPath := writeTeamSetupManifest(t, `version = 3
...
func TestSetupFromTeamManifestRejectsUnreferencedAccountBeforePrompt(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	prompt := &teamSetupPrompt{secrets: []string{"must-not-be-read"}}
	app.Prompt = prompt
...
func TestSetupFromLegacyGenericProfileValidatesBothAccountProtocols(t *testing.T) {
	...
	manifestPath := writeTeamSetupManifest(t, `version = 2
...
func TestSetupFromTeamManifestClientFailureRollsBackCredentialsConfigAndShim(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	app.Prompt = &teamSetupPrompt{secrets: []string{"aigw-test-aihubmix-token", "aigw-test-dmxapi-token"}}
	...
	if _, err := os.Stat(filepath.Join(shimDir, "claude")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("failed setup left Claude shim: %v", err)
	}
...
func TestSetupFromTeamManifestClientFailurePreservesExistingClaudeLauncher(t *testing.T) {
	app, _, secretStore, _ := testApp(t, "")
	app.Interactive = true
	app.Prompt = &teamSetupPrompt{secrets: []string{"aigw-test-aihubmix-token", "aigw-test-dmxapi-token"}}
	app.Runner = &failingRunner{err: errors.New("Codex login failed"), remaining: 1}
	oldShimDir := t.TempDir()
	newShimDir := t.TempDir()
	home := t.TempDir()
	app.Shims.GOOS = "darwin"
	app.Shims.Home = home
	app.Shims.Shell = "/bin/zsh"
	app.Shims.BinDir = oldShimDir
	app.Shims.AIGWExecutable = "/bin/sh"
	if _, err := app.Shims.EnableClaude(); err != nil {
		t.Fatal(err)
	}
	oldShimPath := filepath.Join(oldShimDir, "claude")
	oldActivationPath := filepath.Join(home, ".zshrc")
	oldShim, err := os.ReadFile(oldShimPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	app.Shims.BinDir = newShimDir
	app.Shims.AIGWExecutable = "/bin/echo"
	codexTarget := filepath.Join(t.TempDir(), "config.toml")
	if err := os.WriteFile(codexTarget, []byte("model_provider = \"native\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	gotOldShim, shimErr := os.ReadFile(oldShimPath)
	gotActivation, activationErr := os.ReadFile(oldActivationPath)
	if shimErr != nil || string(gotOldShim) != string(oldShim) || activationErr != nil || string(gotActivation) != string(oldActivation) {
		t.Fatalf("existing Claude launcher state was not restored exactly: shim=%v activation=%v", shimErr, activationErr)
	}
	if _, err := os.Stat(filepath.Join(newShimDir, "claude")); !errors.Is(err, os.ErrNotExist) {
