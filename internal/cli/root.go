...
func NewRoot(app *App) *cobra.Command {
	...
	daily := []*cobra.Command{newUseCommand(app), newRotateCommand(app), newCheckCommand(app), newBalanceCommand(app), newRepairCommand(app), newUpdateCommand(app)}
...
func renderCommandHelp(app *App, command *cobra.Command) {
	...
	if flags.HasAvailableFlags() {
		...
		flags.VisitAll(func(flag *pflag.Flag) {
			...
			r.Row(name, strings.TrimSpace(flag.Usage))
		})
	}
}
